...
export async function fetchCSVData(): Promise<CSVProjectData[]> {
  try {
    ...
    // Organized fallback data with your specific 11 projects
    return [
      ...
      // Group 2: Document Processing & Embargos
      {
      ...
      // Group 3: Document Classification & Liquidations
      {
        'APLICACION': 'Clasificador de Documentos',
        'ESTADO': 'Planificación',
        'AVANCE': '15',
...
export function normalizeProjectData(rawData: CSVProjectData[]): any[] {
  return rawData.map(item => {
    // Handle different possible column names from your CSV
...
export function groupProjectsByCategory(projects: any[]): ProjectGroup[] {
  ...
  projects.forEach(project => {
    ...
    if (grupo.includes('autenticación') || grupo.includes('auth') || project.proyecto.toLowerCase().includes('login')) {
      ...
      // Default assignment based on project name
      if (project.proyecto.toLowerCase().includes('login')) {
        groups[0].projects.push(project)
      } else if (project.proyecto.toLowerCase().includes('clasificador') && project.proyecto.toLowerCase().includes('documento')) {
        // First Clasificador goes to documents group
        if (groups[1].projects.filter(p => p.proyecto.includes('Clasificador')).length === 0) {
          groups[1].projects.push(project)
        } else {
          groups[2].projects.push(project)
        }
      } else if (project.proyecto.toLowerCase().includes('embargo') || 
                 project.proyecto.toLowerCase().includes('pdf') || 
                 project.proyecto.toLowerCase().includes('correo') ||
                 project.proyecto.toLowerCase().includes('supervisor')) {
        groups[1].projects.push(project)
      } else {
        groups[2].projects.push(project)
      }
